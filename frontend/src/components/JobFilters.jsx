...
const JobFilters = ({ onFilterChange, type = 'government' }) => {
  ...
  return (
    <div className="bg-white/90 backdrop-blur-xl border border-stone-200/60 rounded-[2.5rem] p-8 shadow-2xl shadow-stone-200/30 sticky top-24 space-y-8 transition-all duration-500">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-[10px] font-black text-stone-400 hover:text-stone-500 transition-colors uppercase tracking-[0.2em]"
        >
          Reset All
        </button>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="relative group">
          <MapPin className="absolute left-4 top-4 w-4 h-4 text-stone-400 group-focus-within:text-stone-600 transition-colors z-10" />
          <select
            value={filters.location}
            onChange={(e) => handleFilterChange('location', e.target.value)}
            className="w-full pl-12 pr-10 py-4 bg-stone-50 border-2 border-transparent focus:border-stone-500/20 rounded-2xl text-sm font-bold text-stone-900 appearance-none cursor-pointer hover:bg-stone-100 transition-all outline-none"
          >
      ...
      {type === 'private' && (
        <div className="space-y-4">
          <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Job Commitment</label>
      ...
      <div className="space-y-3">
        ...
        <div className="relative group">
          <Building2 className="absolute left-4 top-4 w-4 h-4 text-stone-400 group-focus-within:text-stone-600 transition-colors z-10" />
          <input
            type="text"
            ...
            className="w-full pl-12 pr-4 py-4 bg-stone-50 border-2 border-transparent focus:border-stone-500/20 rounded-2xl text-sm font-bold text-stone-900 outline-none transition-all placeholder:text-stone-300"
          />
        </div>
      </div>
      ...
      {/* Category Grid */}
      <div className="space-y-4">
        <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Industry Focus</label>
        <div className="grid grid-cols-2 gap-3">
          {industryCategories.map((cat) => {
            ...
            return (
              ...
              >
                <Icon className={`w-5 h-5 ${active ? 'text-stone-600' : 'text-stone-400'}`} />
                <span className={`text-[10px] font-black uppercase tracking-tighter ${active ? 'text-stone-700' : 'text-stone-500'}`}>
                  {cat.label}
                </span>
              </button>
            );
