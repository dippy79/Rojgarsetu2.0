                        ...
                        <div key={app.id} className="group bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm hover:shadow-2xl hover:border-stone-400 transition-all cursor-pointer relative overflow-hidden">
                           ...
                           <div className="relative z-10 space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-2 pt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                 {col !== 'Shortlisted' && <button onClick={() => updateStatus(app.id, 'Shortlisted')} className="py-2 bg-stone-50 text-stone-600 rounded-xl text-[9px] font-black uppercase tracking-tighter hover:bg-stone-600 hover:text-white transition-all">Shortlist</button>}
                                 {col !== 'Interview' && <button onClick={() => updateStatus(app.id, 'Interview')} className="py-2 bg-stone-50 text-stone-600 rounded-xl text-[9px] font-black uppercase tracking-tighter hover:bg-stone-600 hover:text-white transition-all">Interview</button>}
                                 {col !== 'Hired' && <button onClick={() => updateStatus(app.id, 'Hired')} className="py-2 bg-stone-50 text-stone-600 rounded-xl text-[9px] font-black uppercase tracking-tighter hover:bg-stone-600 hover:text-white transition-all">Hire</button>}
                                 {col !== 'Rejected' && <button onClick={() => updateStatus(app.id, 'Rejected')} className="py-2 bg-stone-50 text-stone-600 rounded-xl text-[9px] font-black uppercase tracking-tighter hover:bg-stone-600 hover:text-white transition-all">Reject</button>}
                              </div>
                           </div>
                        </div>
                      ))}
...
const getStatusColor = (col) => {
  switch(col) {
    case 'Applied': return 'bg-stone-700';
    case 'Shortlisted': return 'bg-stone-500';
