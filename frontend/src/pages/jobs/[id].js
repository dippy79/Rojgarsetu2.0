...
export default function JobPage(){
  ...
  return (
    <div className="max-w-4xl mx-auto p-10 font-sans">
      <h1 className="text-4xl font-black text-stone-900 mb-4">{job.title || job.data?.title}</h1>
      <div className="bg-white border border-stone-200 rounded-[2rem] p-8 shadow-sm">
         <p className="text-stone-600 leading-relaxed mb-8">{job.description || job.data?.description}</p>
         <a
           href={job.apply_link || job.data?.apply_link}
           target="_blank"
           rel="noreferrer"
           className="inline-block px-10 py-4 bg-stone-900 text-white font-black rounded-2xl hover:bg-stone-600 transition-all uppercase text-xs tracking-widest"
         >
           Apply Now
         </a>
      </div>
