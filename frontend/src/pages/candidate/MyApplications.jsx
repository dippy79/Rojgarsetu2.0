      ...
      <main className="flex-1 overflow-auto p-6 md:p-12 lg:p-16">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-lg font-medium">Tracking {applications.length} deployments through the <span className="text-stone-800 font-bold">Hiring Cycle.</span></p>
          </div>
        </header>
        ...
        <div className="bg-white border border-slate-200 p-6 rounded-[2.5rem] shadow-sm flex flex-col lg:flex-row items-center gap-8 mb-12">
           <div className="flex-1 relative w-full group">
              <Search className="absolute left-5 top-4.5 w-5 h-5 text-slate-400 group-focus-within:text-stone-800 transition-colors" />
              <input
                type="text"
                placeholder="Search by job title or company..."
                value={searchTerm}
                onChange={e => setSearchTerm(e.target.value)}
                className="w-full pl-14 pr-6 py-4.5 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-stone-700/10 transition-all outline-none"
              />
           </div>
           ...
           <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 gap-1 shrink-0 overflow-x-auto max-w-full">
              {['ALL', 'APPLIED', 'SHORTLISTED', 'INTERVIEW', 'REJECTED'].map(f => (
                <button
                  ...
                  className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${statusFilter === f ? 'bg-white text-stone-800 shadow-sm border border-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  {f}
                </button>
              ))}
        ...
        {loading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-[3rem] p-24 text-center space-y-8 shadow-sm">
             ...
             <Link href="/gov-jobs" className="px-10 py-4 bg-slate-900 text-white font-black rounded-[2rem] text-xs uppercase tracking-[0.2em] hover:bg-stone-800 transition-all shadow-2xl shadow-slate-900/10 inline-block">Explore Roles →</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6">
            {filteredApps.map((app) => (
              <div key={app.id} className="group bg-white border border-slate-200 rounded-[2.5rem] p-10 hover:border-stone-400 hover:shadow-2xl hover:shadow-stone-700/5 transition-all relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10">
                ...
                <div className="flex-1 flex flex-col md:flex-row items-center gap-8 relative z-10">
                   <div className="w-20 h-20 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-center text-2xl font-black text-slate-300 group-hover:bg-stone-100 group-hover:border-stone-200 group-hover:text-stone-800 transition-all uppercase">
                   ...
                   <div className="space-y-4 text-center md:text-left">
                      <div className="space-y-1">
                         <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                            <h3 className="text-3xl font-black text-slate-900 group-hover:text-stone-800 transition-colors tracking-tighter leading-tight">{app?.job_title}</h3>
                ...
                <div className="flex flex-col md:flex-row lg:flex-col gap-4 relative z-10 shrink-0 w-full md:w-auto">
                   <button onClick={() => setSelectedApp(app)} className="px-8 py-4 bg-slate-900 text-white font-black rounded-2xl hover:bg-stone-800 transition-all shadow-xl shadow-slate-900/10 uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 group">
               ...
               <div className="space-y-8 relative">
                  ...
                  {[
                    ...
                    <div key={idx} className="flex gap-8 items-start relative z-10">
                       <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-lg ${step.completed ? 'bg-stone-800 text-white shadow-stone-700/20' : 'bg-white border-2 border-slate-100 text-slate-300'}`}>
