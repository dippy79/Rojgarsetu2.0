      ...
      <main className="flex-1 overflow-auto p-6 md:p-12 lg:p-16">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
           <div className="space-y-2">
              <h1 className="text-4xl font-black text-stone-900 tracking-tight">Identity Settings.</h1>
              <p className="text-stone-400 text-lg font-medium">Refining your professional metadata for <span className="text-stone-800 font-bold">AI Ingestion.</span></p>
           </div>

           <div className="flex items-center gap-8">
              <div className="text-right">
                 <p className="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1">Profile Strength</p>
                 <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-black text-stone-900">{completionPercentage}%</span>
                 </div>
              </div>
           </div>
        </header>
        ...
        {loading ? (
          <div className="space-y-12 animate-pulse">
             <div className="h-96 bg-white border border-stone-200 rounded-[3rem]"></div>
             <div className="h-64 bg-white border border-stone-200 rounded-[3rem]"></div>
          </div>
        ) : (
          <form onSubmit={handleSaveProfile} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
             <div className="lg:col-span-8 space-y-8">
                {/* Basic Intel Section */}
                <div className="bg-white border border-stone-200 rounded-[3.5rem] p-12 shadow-sm space-y-10">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                         <label htmlFor="full_name" className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Legal Name</label>
                         <input id="full_name" name="full_name" type="text" value={profile.full_name} onChange={e => setProfile({...profile, full_name: e.target.value})} className="w-full px-6 py-4 bg-stone-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-stone-700/10 transition-all outline-none" placeholder="Simranjeet Singh" autocomplete="name" />
                      </div>
                      <div className="space-y-3">
                         <label htmlFor="email" className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Sync Address</label>
                         <input id="email" name="email" type="email" value={profile.email} disabled className="w-full px-6 py-4 bg-stone-100 border-none rounded-2xl text-sm font-bold text-stone-400 cursor-not-allowed outline-none" autocomplete="email" />
                      </div>
                   </div>

                   <div className="space-y-3">
                      <label htmlFor="title" className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Professional Header</label>
                      <input id="title" name="title" type="text" value={profile.title} onChange={e => setProfile({...profile, title: e.target.value})} className="w-full px-6 py-4 bg-stone-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-stone-700/10 transition-all outline-none" placeholder="Senior DevOps Architect" autocomplete="organization-title" />
                   </div>

                   <div className="space-y-3">
                      <label htmlFor="bio" className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">Executive Summary</label>
                      <textarea id="bio" name="bio" rows={5} value={profile.bio} onChange={e => setProfile({...profile, bio: e.target.value})} className="w-full px-6 py-4 bg-stone-50 border-none rounded-[2rem] text-sm font-bold focus:ring-4 focus:ring-stone-700/10 transition-all outline-none resize-none" placeholder="Deep-dive into your trajectory..." autocomplete="off" />
                   </div>
                </div>
                   ...
                   <div className="space-y-6">
                      <div className="flex flex-wrap gap-3">
                         {profile.skills.map(s => (
                           <div key={s.id} className="flex items-center gap-3 px-5 py-2.5 bg-stone-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest border border-stone-800 shadow-xl shadow-stone-900/10 transition-all hover:scale-105">
                              {s.name}
                      ...
                      <div className="relative group">
                         <Plus className="absolute left-5 top-4.5 w-5 h-5 text-stone-400" />
                         <input
                           type="text"
                           ...
                           className="w-full pl-14 pr-6 py-5 bg-stone-50 border-none rounded-3xl text-sm font-bold focus:ring-4 focus:ring-stone-500/10 transition-all outline-none"
                         />
                      </div>
                   </div>
                </div>
             ...
             <div className="lg:col-span-4 space-y-8">
                {/* Logistics Bento */}
                <div className="bg-stone-900 rounded-[3rem] p-10 text-white space-y-8 shadow-2xl shadow-blue-900/20 relative overflow-hidden">
                   <div className="absolute top-0 right-0 w-32 h-32 bg-stone-800/10 rounded-full -mr-16 -mt-16 blur-3xl"></div>

                   <div className="space-y-6 relative z-10">
                      <div className="space-y-3">
                         <label htmlFor="location" className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1">Current Hub</label>
                         <div className="relative group">
                            <MapPin className="absolute left-4 top-4 w-4 h-4 text-stone-600 group-focus-within:text-stone-400 transition-colors" />
                      ...
                      <div className="space-y-3">
                         <label htmlFor="phone" className="text-[10px] font-black text-stone-500 uppercase tracking-widest ml-1">Direct Line</label>
                         <div className="relative">
                            <Phone className="absolute left-4 top-4 w-4 h-4 text-stone-600" />
                ...
                {/* Social Nodes */}
                <div className="bg-white border border-stone-200 rounded-[3rem] p-10 space-y-6">
                   <h4 className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Digital Footprint</h4>
                   <div className="space-y-4">
                      <div className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl border border-stone-100 group transition-all hover:border-stone-900">
                         <Github className="w-5 h-5 text-stone-400 group-hover:text-stone-900 transition-colors" />
                      ...
                      <div className="flex items-center gap-4 p-4 bg-stone-50 rounded-2xl border border-stone-100 group transition-all hover:border-stone-800">
                         <Linkedin className="w-5 h-5 text-stone-400 group-hover:text-stone-800 transition-colors" />
