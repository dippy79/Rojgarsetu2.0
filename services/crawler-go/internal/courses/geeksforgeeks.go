...
//
// It implements the shared.CourseFetcher interface (Fetch + Name) and writes into the
// same shared.CourseSource model used by nptel.go, swayam.go, nsdc.go, coursera.go and
...
func NewGeeksforGeeksSource() *GeeksforGeeksSource {
	return &GeeksforGeeksSource{
		BaseSource: shared.BaseSource{NameStr: "geeksforgeeks", BaseURL: "https://www.geeksforgeeks.org"},
		client: &http.Client{
...
func (s *GeeksforGeeksSource) Fetch(ctx context.Context) ([]shared.CourseSource, error) {
...
func (s *GeeksforGeeksSource) parseCourses(html string) []shared.CourseSource {
...
func (s *GeeksforGeeksSource) parseJSONLDCourses(html string) []shared.CourseSource {
	var courses []shared.CourseSource
...
func (s *GeeksforGeeksSource) jsonLDToCourse(c gfGCourse) *shared.CourseSource {
	...
	course := &shared.CourseSource{
		...
		URL:         url,
		Description: shared.CleanString(c.Description),
...
func (s *GeeksforGeeksSource) parseHTMLCourses(html string) []shared.CourseSource {
	var courses []shared.CourseSource
	...
	for _, pattern := range patterns {
		...
		for _, match := range matches {
			...
			if len(match) >= 3 {
				...
				title = shared.CleanString(match[1])
			}
