...
func NewLeverSource() *LeverSource {
	return &LeverSource{
		BaseSource: shared.BaseSource{NameStr: "lever", BaseURL: "https://api.lever.co"},
		client: &http.Client{
...
func (s *LeverSource) Fetch(ctx context.Context) ([]shared.PrivJobSource, error) {
...
func (s *LeverSource) fetchOrg(ctx context.Context, org leverOrg) ([]shared.PrivJobSource, error) {
...
// postingToPriv converts a Lever posting to the shared shared.PrivJobSource model.
func (s *LeverSource) postingToPriv(p *leverPosting, company string) shared.PrivJobSource {
	job := shared.PrivJobSource{
		...
		URL:         p.HostedURL,
		JobType:     shared.NormalizeJobType(p.Categories.Commitment),
		Description: "",
		CreatedAt:   time.Now(),
	}
