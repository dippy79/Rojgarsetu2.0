...
func NewAuthService(database *db.PostgresDB, userSvc *UserService, cfg *config.Config) *AuthService {
    s := &AuthService{
        db:            database,
        userSvc:       userSvc,
        cfg:           cfg,
...
func (s *AuthService) Refresh(c *gin.Context) {
    ...
    if err = s.db.RevokeRefreshToken(c, refreshID); err != nil {
...
func (s *AuthService) LogoutAll(c *gin.Context) {
    ...
    if err = s.db.RevokeAllTokensForUser(c, userID); err != nil {
