-- RojgarSetu 2.0 - New Database Schema
...
-- ============================================
-- CRAWLER LOGS TABLE (for monitoring)
-- ============================================
...
-- ============================================
CREATE TRIGGER update_jobs_government_updated_at 
    BEFORE UPDATE ON jobs_government 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_jobs_private_updated_at 
    BEFORE UPDATE ON jobs_private 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- View for active government jobs
CREATE OR REPLACE VIEW v_active_gov_jobs AS
SELECT 
    id, title, department, location, apply_url, last_date, 
    source, eligibility, vacancy_count, salary, exam_date,
    notification_pdf_url, created_at
FROM jobs_government
WHERE is_active = true
ORDER BY created_at DESC;

-- View for active private jobs
CREATE OR REPLACE VIEW v_active_priv_jobs AS
SELECT 
    id, company, title, location, url, salary, experience,
    job_type, skills, description, source, posted_at, created_at
FROM jobs_private
WHERE is_active = true
ORDER BY created_at DESC;

-- View for active courses
CREATE OR REPLACE VIEW v_active_courses AS
SELECT 
    id, provider, title, url, duration, mode, level,
    skills, description, thumbnail_url, price, is_free,
    source, start_date, end_date, enrollment_count, rating,
    created_at
FROM courses
WHERE is_active = true
ORDER BY created_at DESC;

-- View for active YouTube videos
CREATE OR REPLACE VIEW v_active_yt_videos AS
SELECT 
    id, channel, channel_id, title, url, thumbnail,
...
-- ============================================
-- SEQUENCES FOR AUTO-INCREMENT (if using int IDs)
-- ============================================
-- Note: Using UUID as primary key, but keeping sequences for compatibility
...
-- ============================================
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA PUBLIC TO rojgarsetu;
-- GRANT USAGE ON ALL SEQUENCES IN SCHEMA PUBLIC TO rojgarsetu;
